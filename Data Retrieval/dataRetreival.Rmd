...
test_date <- as.POSIXct("2020-05-06 00:01:00")
...
days_in_month <- function(month) {
  ...
  end_date <- as.Date(format(seq(start_date, by = "month", length.out = 2)[2] - 1, "%Y-%m-%d"))
  return(as.integer(format(end_date, "%d")))
}

# Function to fetch rainfall data for a specific timestamp
get_rainfall_data_hour <- function(timestamp) {
  response <- GET(base_url, query = list(date_time = timestamp))
  if (status_code(response) == 200) {
    # Parse the JSON content
    data <- content(response, as = "text", encoding = "UTF-8")
    json_data <- fromJSON(data, flatten = TRUE)
    
    # Check if 'items' exists and has data
    if ("items" %in% names(json_data) && length(json_data$items) > 0) {
      # Extract data for each timestamp and readings
      rainfall_data <- lapply(seq_along(json_data$items$timestamp), function(i) {
        # Extract timestamp and readings
        timestamp <- json_data$items$timestamp[i]
        readings <- json_data$items$readings[[i]]
        
        # Convert readings to a data frame and add the timestamp column
        if (!is.null(readings) && is.data.frame(readings)) {
          readings_df <- readings %>%
            mutate(timestamp = timestamp) %>%
            select(timestamp, station_id, rainfall = value)
          return(readings_df)
        } else {
          return(NULL)
        }
      })
      return(bind_rows(Filter(Negate(is.null), rainfall_data)))
    } else {
      print(paste("No data for timestamp:", timestamp))
      return(NULL)
    }
  } else {
    print(paste("Error fetching data for timestamp:", timestamp))
    return(NULL)
  }
}

rainfall_2023 <- list()  # Initialize an empty list to store each month's data

for (month in 1:12) {

  start_date <- as.POSIXct(sprintf("2023-%02d-01 00:00:00", month))
  end_date <- as.POSIXct(sprintf("2023-%02d-%02d 23:00:00", month, days_in_month(month)))
  timestamps_month <- seq(start_date, end_date, by = "hour")
  timestamps_month <- format(timestamps_month, "%Y-%m-%dT%H:%M:%S")

  month_data <- bind_rows(lapply(timestamps_month, get_rainfall_data_hour))

  rainfall_2023[[month]] <- month_data
  
  Sys.sleep(0.1)  # Adjust as necessary
}

# Combine all monthly data into a single data frame
rainfall_2023_df <- bind_rows(rainfall_2023)

# View the data if available
if (!is.null(rainfall_2023_df)) {
  head(rainfall_2023_df)
} else {
  print("No rainfall data for the year 2023.")
}
```
...
cp_extract_data <- function(datetime, url){
  ...
  carpark <- carpark[carpark_number == "CR6" | carpark_number == "Y77L" | carpark_number == "W56L"]
...
```{r}
query_date_seq <- seq(as.POSIXct("2023-06-09 00:01:00"),
                      as.POSIXct("2024-08-01 00:01:00"),
                      by = "hour")
...
dm_carpark_data <- dm_carpark_historic_data_get(query_date_seq)
dm_carpark_data_res <- dm_carpark_data$results
dm_carpark_data_res[CarParkID == "2"]

dm_carpark_errors <- dm_carpark_data$error
errors <- do.call(c, lapply(dm_carpark_errors, `[[`, 1))
range(errors)

dm_carpark_historic_data_get(as.POSIXct("2023-06-09 00:01:00"))
